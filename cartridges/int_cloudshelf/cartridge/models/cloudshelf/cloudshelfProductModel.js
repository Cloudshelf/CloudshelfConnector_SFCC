...
function product(productSearchHit) {
    if (
        ...
        const gid = cloudshelfHelper.getGlobalId(
            cloudshelfHelper.GLOBAL_ID_NAMESPACES.PRODUCT,
            productSearchHit.product.ID,
        );

        this.description = String(
            productSearchHit.product.shortDescription || productSearchHit.product.longDescription || '',
        );
        this.displayName = productSearchHit.product.name;
        this.id =
            productSearchHit.representedProducts.length > 1
                ? gid
                : cloudshelfHelper.getGlobalId(
                      cloudshelfHelper.GLOBAL_ID_NAMESPACES.PRODUCT,
                      productSearchHit.product.ID + 'M',
                  );
        this.metadata = cloudshelfHelper.getMetadata(productSearchHit.product, 'cloudshelfProductMetadataMapping');
        this.productType =
            productSearchHit.product.primaryCategory != null
                ? productSearchHit.product.primaryCategory.displayName
                : productSearchHit.product.allCategoryAssignments[0].category.displayName;
